# F46 Requirements

## Functional

1. Shared portal shell supports an onboarding-specific header mode.
2. Signup, plan, and billing routes use onboarding header mode.
3. Onboarding header mode hides full utility navigation and keeps a focused conversion frame.
4. Workspace routes retain full operator chrome behavior.

## Non-Functional

1. No API contract changes.
2. Existing signup/workspace QA scripts remain green.
3. Build and targeted backend tests remain green.
